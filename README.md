<div align="center">

<img src="./onedisc.png" width="180" height="180" />

# OneDisc


Discord 平台的 OneBot 实现

[![Badge](https://img.shields.io/badge/OneBot-12-black?logo=data:image/png;base64,)](https://12.onebot.dev)
[![Build (Dev)](https://github.com/ITCraftDevelopmentTeam/OneDisc/actions/workflows/build-beta.yml/badge.svg)](https://github.com/ITCraftDevelopmentTeam/OneDisc/actions/workflows/build-beta.yml)

</div>

> [!WARNING]
> __屎山警告__: 虽然我们已经在努力重构代码，但本项目仍然有非常多诸如 抽象的命名、写了跟没写一样的注释、乱七八糟的导入 的内容，请在阅读时注意控制您的血压。
